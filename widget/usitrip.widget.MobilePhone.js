 ...
 */
 ...
 (function(){
	...
	usitrip.staticResource.hotArea = ['中国','美国','加拿大','香港','澳门','台湾','新加坡'];
...
(function(){
	...
	Usitrip.extend(usitrip.widget.MobilePhoneField, usitrip.event.UsitripEvent, {
		...
		doRender: function(el){
			...
			content += '</dl></div>';
			content += '<div class="area-box"><div class="areas"><i class="flag"></i><input name="' + this.areaname + '" class="areaname" disabled="disabled" value="' + zh + '"><em><!– –></em></div> <input name="' + this.name + '" class="phone" type="text" value="' + phoneInit + '" placeholder="' + this.initValue.placeholder + '" /><i class="tips">' + this.tipText +'</i></div>'
				+ '</div>';
			$(el).html(content);
		...
		doEvent: function(){
			...
			var fun = function(event){
				...
				this.updateBgImgPosition(this.areaFlag, code, zh);
				this.areaName.val(zh);
				this.areaPhone.val(code + ' ');
			}
            ...
            this.areaPhone.change($.proxy(this.inputComplete, this));
		...
		inputComplete: function(event){
			...
			if(/^[\+\s\d]+$/.test(value)){ //原正则：/^\+\d+\s+\d+$/ by j 时间：16-3-30
				var len = value.substr(value.indexOf(' ') + 1).replace(/\s/g, '').length;
        ...
        addPhoneClass: function(clazz){
            this.areaPhone.addClass(clazz);
        },

        addAreaClass: function(clazz){
            this.areaName.addClass(clazz);
        }
	});
})();